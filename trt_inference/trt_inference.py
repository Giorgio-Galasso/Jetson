...
import pycuda.autoinit  # Inizializza automaticamente il contesto GPU
import numpy as np
import time
import os
...
def load_engine(plan_path):
    """Carica il file .plan dalla memoria."""
...
def main():
    ...
    # 4. Sincronizza (attende che la GPU finisca)
    stream.synchronize()
    ...
    # ANALISI RISULTATI
    # ==========================================
    # Cerchiamo il buffer di output
    output_key = "output" # o il nome che hai dato all'output
    ...
    print(f"⏱️  Latenza inferenza: {latenza:.2f} ms")
    print(f"🤖 Azione predetta: {azione}")
    print(f"📈 Logits grezzi: {output_logits[:5]}...") # Mostra i primi 5 valori
    print("-" * 40)
